...
class ServerController {
    ...
    def list = {
        ...
        if(params?.format && params.format != "html"){
            ...
            response.setHeader("Content-disposition", "attachment; filename=servers.${params.extension}")

            exportService.export(params.format, response.outputStream,Server.list(params), [:], [:])
        }
    ...
    def save = {
        ...
        if (serverInstance.save(flush: true)) {
            flash.message = "${message(code: 'default.created.message', args: [message(code: 'server.label', default: 'Server'), serverInstance.hostname])}"
