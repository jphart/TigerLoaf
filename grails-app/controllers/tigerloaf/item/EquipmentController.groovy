...
class EquipmentController {
    ...
    def list = {
        params.max = Math.min(params.max ? params.int('max') : 10, 100)
         ...
         if(params?.format && params.format != "html"){
            ...
            response.setHeader("Content-disposition", "attachment; filename=equipment.${params.extension}")
    ...
    def save = {
        ...
        if (equipmentInstance.save(flush: true)) {
            flash.message = "${message(code: 'default.created.message', args: [message(code: 'equipment.label', default: 'Equipment'), equipmentInstance.itemName])}"
